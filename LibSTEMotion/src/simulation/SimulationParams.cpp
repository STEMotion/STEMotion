...
namespace STEMotion {
    ...
    BaseQuantity const& SimulationParams::get_metric_param(const char *name) {
        return *params[name];
    }

    bool SimulationParams::has_metric_param(const char *name) const {
    ...
    Time const& SimulationParams::get_time_param(const char *name) {
        return *time_params[name];
    }

    bool SimulationParams::has_time_param(const char *name) const {
        return time_params.contains(name);
    }
    ...
    Angle const& SimulationParams::get_angle_param(const char *name) {
        return *angle_params[name];
    }

    bool SimulationParams::has_angle_param(const char *name) const {
        return angle_params.contains(name);
    }
